package solutions;
/**************************
 *  Compilation:  javac main.solutions.QuickX.java
 *  Execution:    java main.solutions.QuickX < input.txt
...
/**
 * The {@code main.solutions.QuickX} class provides static methods for sorting an array
...
public class QuickX {
    ...
    private static int partition(Comparable[] a, int lo, int hi) {
        ...
        int k = median5(a, lo, (lo + n / 2) - 1, lo + n / 2, (lo + n / 2) + 1, hi);
        exch(a, k, lo);
    ...
    private static int median5(Comparable[] a, int i, int j, int k, int l, int n) {
        Comparable[] sorted = new Comparable[]{a[i], a[j], a[k], a[l], a[n]};
        Insertion.sort(sorted);
        Comparable median = sorted[2];
        if (median == a[i])
            return i;
        else if (median == a[j])
            return j;
    ...
    public static void main(String[] args) {
        Comparable[] test = {2, 3, 3, 1, 15, 9, 23, 1, 42, 3};
        sort(test);
        show(test);
    }
